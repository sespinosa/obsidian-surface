...
export async function thoughtRecent(p: { limit?: number }): Promise<ToolResult> {
...
export async function thoughtEnrich(p: {
  ...
  await execObsidian(["create", `path=${p.path}`, `content=${newContent}`, "overwrite"]);

  // Update index entry
  const tagsArray = p.tags || (fm["tags"] ? fm["tags"].replace(/[\[\]]/g, "").split(",").map((s) => s.trim()).filter(Boolean) : undefined);
  await appendEntry({
    path: p.path,
...
- project_rename: Rename a project (from, to required)`,
    {
      action: z
        .enum([
          "create", "list", "index", "recent", "enrich", "reindex",
          "clear", "project_set", "project_list", "project_rename",
        ])
      ...
      summary: z.string().optional().describe("Brief summary of the thought (for create, enrich)"),
      ...
      since: z.string().optional().describe("ISO date filter for index (e.g. 2025-01-01)"),
      query: z.string().optional().describe("Text search filter for index"),
      limit: z.number().optional().describe("Max results for recent (default 10)"),
      cwd_override: z.string().optional().describe("Override working directory recorded in frontmatter"),
    },
    async (params) => {
      try {
